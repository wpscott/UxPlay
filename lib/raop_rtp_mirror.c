...
raop_rtp_mirror_thread(void* arg)
{
	...
	while (1)
	{
			...
			FD_ISSET(raop_rtp_mirror->mirror_data_sock, &rfds))
		{
			...
			{
				logger_log(raop_rtp_mirror->logger, LOGGER_WARNING,
				           "raop_rtp_mirror could not set stream socket keepalive interval %d %s", errno,
				           strerror(errno));
			}
			option = 6;
			...
			{
				logger_log(raop_rtp_mirror->logger, LOGGER_WARNING,
				           "raop_rtp_mirror could not set stream socket keepalive probes %d %s", errno,
				           strerror(errno));
			}
			readstart = 0;
		}
		...
		if (stream_fd != -1 && FD_ISSET(stream_fd, &rfds))
		{
			...
			if (payload == NULL && ret == 0)
			{
				logger_log(raop_rtp_mirror->logger, LOGGER_DEBUG,
				           "raop_rtp_mirror tcp socket was closed by client (recv returned 0); got %d bytes of 128 byte header",
				           readstart);
			...
			else if (payload == NULL && ret == -1)
			{
				...
				// Timeouts can happen even if the connection is fine
				logger_log(raop_rtp_mirror->logger, LOGGER_ERR,
			...
			{
				logger_log(raop_rtp_mirror->logger, LOGGER_ERR,
				           "raop_rtp_mirror tcp socket was closed by client (recv returned 0)");
				break;
			}
			else if (ret == -1)
			{
				if (errno == EAGAIN || errno == EWOULDBLOCK) continue;
				// Timeouts can happen even if the connection is fine
				logger_log(raop_rtp_mirror->logger, LOGGER_ERR, "raop_rtp_mirror error in recv: %d %s", errno,
				           strerror(errno));
				if (errno == ECONNRESET) conn_reset = true;
				break;
			}
			...
			// start code for the NAL Byte-Stream Format.
				...
				while (nalu_size < payload_size)
				{
					...
					case 6:
						if (logger_debug)
						{
							...
							logger_log(raop_rtp_mirror->logger, LOGGER_DEBUG, "raop_rtp_mirror SEI NAL size = %d",
							           nc_len);
							logger_log(raop_rtp_mirror->logger, LOGGER_DEBUG,
					...
					case 7:
						...
						{
							char* str = utils_data_to_string(payload_decrypted + nalu_size, nc_len, 16);
							logger_log(raop_rtp_mirror->logger, LOGGER_DEBUG, "raop_rtp_mirror SPS NAL size = %d",
							           nc_len);
							logger_log(raop_rtp_mirror->logger, LOGGER_DEBUG,
					...
					case 8:
						...
						{
							char* str = utils_data_to_string(payload_decrypted + nalu_size, nc_len, 16);
							logger_log(raop_rtp_mirror->logger, LOGGER_DEBUG, "raop_rtp_mirror PPS NAL size = %d",
							           nc_len);
							logger_log(raop_rtp_mirror->logger, LOGGER_DEBUG,
				...
				raop_rtp_mirror->callbacks.video_process(raop_rtp_mirror->callbacks.cls, raop_rtp_mirror->ntp,
				                                         &h264_data);
				free(payload_out);
				break;
			case 0x01:
				...
				if (width != width_source || height != height_source)
				{
					logger_log(raop_rtp_mirror->logger, LOGGER_DEBUG, "raop_rtp_mirror: Unexpected : data  %f,"
					           " %f != width_source = %f, height_source = %f", width, height, width_source,
					           height_source);
				}
				width = byteutils_get_float(packet, 48);
				height = byteutils_get_float(packet, 52);
				logger_log(raop_rtp_mirror->logger, LOGGER_DEBUG,
				           "raop_rtp_mirror: unidentified extra header data  %f, %f", width, height);
				width = byteutils_get_float(packet, 56);
				height = byteutils_get_float(packet, 60);
				if (raop_rtp_mirror->callbacks.video_report_size)
				{
					raop_rtp_mirror->callbacks.video_report_size(raop_rtp_mirror->callbacks.cls, &width_source,
					                                             &height_source, &width, &height);
				}
				logger_log(raop_rtp_mirror->logger, LOGGER_DEBUG,
				...
				if (logger_debug)
				{
					...
					logger_log(raop_rtp_mirror->logger, LOGGER_DEBUG, "raop_rtp_mirror: SPS+PPS header size = %d",
					           data_size);
				...
				{
					logger_log(raop_rtp_mirror->logger, LOGGER_ERR, " pps_sps error: packet remainder size = %d < 0",
					           data_size);
				}
			...
			// Copy the sps and pps into a buffer to prepend to the next NAL unit.
				...
				{
					logger_log(raop_rtp_mirror->logger, LOGGER_WARNING,
					           "ntp synchronization has not yet started: synchronized video may fail");
				}
			// h264codec_t h264;
			...
			case 0x02:
				logger_log(raop_rtp_mirror->logger, LOGGER_DEBUG,
				           ...
				           " payload_size %d header %s ts_raw = %llu", payload_size, packet_description,
				           ntp_timestamp_raw);
