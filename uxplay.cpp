...
static FILE* video_dumpfile = nullptr;
...
static FILE* audio_dumpfile = nullptr;
...
static unsigned short display[5] = {}, tcp[3] = {}, udp[3] = {};
...
static size_t write_coverart(const char* filename, const void* image, const size_t len)
{
	FILE* fp;
	fopen_s(&fp, filename, "wb");
	const size_t count = fwrite(image, 1, len, fp);
	fclose(fp);
	return count;
}

static char* create_pin_display(const char* pin_str, const int margin, const int gap)
{
	char* ptr;
	char num[2] = {};
	constexpr int w = 10;
	constexpr int h = 8;
	const char digits[10][8][11] = {
		{
			"0821111380", "2114005113", "1110000111", "1110000111", "1110000111", "1110000111", "5113002114",
			"0751111470"
		},
		{
			"0002111000", "0021111000", "0000111000", "0000111000", "0000111000", "0000111000", "0000111000",
			"0011111110"
		},
		{
			"0811112800", "2114005113", "0000000111", "0000082114", "0862111470", "2114700000", "1117000000",
			"1111111111"
		},
		{
			"0821111380", "2114005113", "0000082114", "0000111170", "0000075130", "1110000111", "5113002114",
			"0751111470"
		},
		{
			"0000211110", "0001401110", "0021401110", "0214001110", "2110001110", "1111111111", "0000001110",
			"0000001110"
		},
		{
			"1111111110", "1110000000", "1110000000", "1112111380", "0000075113", "0000000111", "5113002114",
			"0711114700"
		},
		{
			"0821111380", "2114005113", "1110000000", "1112111380", "1114075113", "1110000111", "5113002114",
			"0751111470"
		},
		{
			"1111111111", "0000002114", "0000021140", "0000211400", "0002114000", "0021140000", "0211400000",
			"2114000000"
		},
		{
			"0831111280", "2114002114", "5113802114", "0751111170", "8214775138", "1110000111", "5113002114",
			"0751111470"
		},
		{"0821111380", "2114005113", "1110000111", "5113802111", "0751114111", "0000000111", "5113002114", "0751111470"}
	};

	constexpr char pixels[9] = {' ', '8', 'd', 'b', 'P', 'Y', 'o', '"', '.'};
	/* Ascii art used here is derived from the FIGlet font "collosal" */

	int pin_val = static_cast<int>(strtoul(pin_str, &ptr, 10));
	if (*ptr)
	{
		return nullptr;
	}
	const int len = strlen(pin_str);
	const auto pin = static_cast<int*>(calloc(len, sizeof(int)));
	if (!pin)
	{
		return nullptr;
	}
	...
	const int size = 4 + h * (margin + len * (w + gap + 1));
	const auto pin_image = static_cast<char*>(calloc(size, sizeof(char)));
	if (!pin_image)
	{
		return nullptr;
	}
	...
	for (int i = 0; i < h; i++)
	{
		...
		{
			const int l = pin[j];
			const char* p = digits[l][i];
			for (int k = 0; k < w; k++)
			{
				char* ptr;
				strncpy_s(num, p++, 1);
				const int r = static_cast<int>(strtoul(num, &ptr, 10));
...
static void dump_audio_to_file(const unsigned char* data, const int datalen, unsigned char type)
{
	if (!audio_dumpfile && audio_type != previous_audio_type)
	{
		...
		fn.append(suffix);
		fopen_s(&audio_dumpfile, fn.c_str(), "w");
		if (audio_dumpfile == nullptr)
		{
...
static void dump_video_to_file(const unsigned char* data, const int datalen)
{
	...
	if (!video_dumpfile)
	{
		...
		fn.append(".h264");
		if (fopen_s(&video_dumpfile, fn.c_str(), "w"))
		{
...
static gboolean reset_callback(const gpointer loop)
{
	if (reset_loop)
	{
...
static gboolean sigint_callback(const gpointer loop)
{
...
static gboolean sigterm_callback(const gpointer loop)
{
	relaunch_video = false;
	g_main_loop_quit(static_cast<GMainLoop*>(loop));
	return TRUE;
...
static guint g_unix_signal_add(const gint signum, const GSourceFunc handler, const gpointer user_data)
{
...
static void main_loop()
{
	...
	{
		relaunch_video = true;
		gst_bus_watch_id = video_renderer_listen(loop);
	}
	const guint reset_watch_id = g_timeout_add(100, reset_callback, loop);
	const guint sigterm_watch_id = g_unix_signal_add(SIGTERM, sigterm_callback, loop);
	const guint sigint_watch_id = g_unix_signal_add(SIGINT, sigint_callback, loop);
...
{
	for (int i = 0; i < static_cast<int>(str.length()); i += 3)
	{
		hw_addr.push_back(static_cast<char>(stol(str.substr(i), nullptr, 16)));
	}
	return 0;
}
...
static const char* get_homedir()
{
	char* homedir;
	size_t len;
	auto err = _dupenv_s(&homedir, &len, "XDG_CONFIG_HOMEDIR");
	if (err)
	{
		err = _dupenv_s(&homedir, &len, "HOME");
	}
#ifndef _WIN32
	if (homedir == NULL) {
...
static std::string find_uxplay_config_file()
{
	...
	struct stat sb;
	_dupenv_s(&uxplayrc, &len, "UXPLAYRC"); /* first look for $UXPLAYRC */
	if (uxplayrc)
	{
		config0 = uxplayrc;
...
static std::string find_mac()
{
	...
	auto addresses = static_cast<IP_ADAPTER_ADDRESSES*>(malloc(buflen));
	...
	{
		free(addresses);
		addresses = static_cast<IP_ADAPTER_ADDRESSES*>(malloc(buflen));
		if (addresses == nullptr)
		{
			return mac;
		}
	...
	{
		for (PIP_ADAPTER_ADDRESSES address = addresses; address != nullptr; address = address->Next)
		{
			...
			{
				char str[3];
				snprintf(str, sizeof(str), "%02x", static_cast<int>(address->PhysicalAddress[i]));
				mac.append(str);
				if (i < 5) mac.append(":");
			}
			break;
...
static bool validate_mac(const char* mac_address)
{
	char c;
...
static bool get_display_settings(std::string value, unsigned short* w, unsigned short* h, unsigned short* r)
{
	...
	*w = static_cast<unsigned short>(strtoul(value.c_str(), &end, 10));
	if (*end || *w == 0) return false;
	pos = str1.find_first_of("@");
	if (pos != std::string::npos)
	{
		const std::string str2 = str1.substr(pos + 1);
		if (str2.length() == 0 || str2.length() > 3 || str2[0] == '-') return false;
		*r = static_cast<unsigned short>(strtoul(str2.c_str(), &end, 10));
		if (*end || *r == 0 || *r > 255) return false;
		str1.erase(pos);
	}
	if (str1.length() == 0 || str1.length() > 4 || str1[0] == '-') return false;
	*h = static_cast<unsigned short>(strtoul(str1.c_str(), &end, 10));
...
static bool get_value(const char* str, unsigned int* n)
{
	...
	char* end;
	const unsigned long l = strtoul(str, &end, 10);
	if (*end) return false;
...
static bool get_ports(const int nports, std::string option, const char* value, unsigned short* const port)
{
	...
	for (int i = 0; i <= nports; i++)
	{
		...
		*(port + i) = static_cast<unsigned short>(l);
		for (int j = 0; j < i; j++)
		{
...
static void parse_arguments(const int argc, char* argv[])
{
	...
	for (int i = 1; i < argc; i++)
	{
		...
		if (arg == "-allow")
		{
			...
			i++;
			allowed_clients.emplace_back(argv[i]);
		}
		else if (arg == "-block")
		{
			if (!option_has_value(i, argc, arg, argv[i + 1])) exit(1);
			i++;
			blocked_clients.emplace_back(argv[i]);
		}
		...
		else if (arg == "-async")
		{
			...
			if (i < argc - 1)
			{
				...
				char* end;
				const int n = static_cast<int>(strtof(argv[i + 1], &end) * 1000);
				if (*end == '\0')
				{
					i++;
		...
		else if (arg == "-fps")
		{
			...
			{
				fprintf(stderr, "invalid \"-fps %s\"; -fps n : max n=255, default n=60\n", argv[i]);
				exit(1);
			}
			display[3] = static_cast<unsigned short>(n);
		}
		else if (arg == "-o")
		{
			display[4] = 1;
		...
		else if (arg == "-al")
		{
			...
			if (i < argc - 1 && *argv[i + 1] != '-')
			{
				n = static_cast<int>(strtof(argv[++i], &end) * SECOND_IN_USECS);
		...
		else if (arg == "-db")
		{
			...
			if (db_bad)
			{
				fprintf(
					stderr,
					"invalid %s %s: db value must be \"low\" or \"low:high\", low < 0 and high > low are decibel gains\n",
					argv[i], start);
				exit(1);
			}
			i++;
...
static void process_metadata(const int count, const char* dmap_tag, const unsigned char* metadata, const int datalen)
{
	int dmap_type = 0;
	...
	if (dmap_type == 9)
	{
		const auto str = static_cast<char*>(calloc(1, datalen + 1));
	...
	else if (debug_log)
	{
		for (int i = 0; i < datalen; i++)
		{
			...
			printf("%2.2x ", static_cast<int>(metadata[i]));
		}
	}
	printf("\n");
}
...
{
	int dnssd_error;
	const int require_pw = (require_password ? 1 : 0);
	if (dnssd)
	{
	...
	*/
	...
	dnssd_set_airplay_features(dnssd, 27, setup_legacy_pairing);
	return 0;
}

static bool check_client(const char* deviceid)
{
	bool ret = false;
	const int list = allowed_clients.size();
...
static bool check_blocked_client(const char* deviceid)
{
	bool ret = false;
	const int list = blocked_clients.size();
	for (int i = 0; i < list; i++)
	{
...
extern "C" void export_dacp(void* cls, const char* active_remote, const char* dacp_id)
{
	...
	{
		FILE* fp;
		if (fopen_s(&fp, dacpfile.c_str(), "w") == 0)
		{
...
extern "C" void conn_reset(void* cls, const int timeouts, const bool reset_video)
{
...
extern "C" void audio_process(void* cls, raop_ntp_t* ntp, audio_decode_struct* data)
{
	...
	if (use_audio)
	{
		...
		case 2:
			if (audio_delay_alac)
			{
				data->ntp_time_remote = static_cast<uint64_t>((int64_t)data->ntp_time_remote + audio_delay_alac);
			}
			break;
		case 4:
		case 8:
			if (audio_delay_aac)
			{
				data->ntp_time_remote = static_cast<uint64_t>((int64_t)data->ntp_time_remote + audio_delay_aac);
			}
			break;
		default:
			break;
...
extern "C" void audio_set_volume(void* cls, const float volume)
{
	...
	{
		frac = static_cast<double>((30.0f + volume) / 30.0f);
...
extern "C" void audio_get_format(void* cls, unsigned char* ct, unsigned short* spf, bool* usingScreen,
                                 bool* isMedia,
                                 uint64_t* audioFormat)
{
	unsigned char type;
	LOGI("ct=%d spf=%d usingScreen=%d isMedia=%d  audioFormat=0x%lx", *ct, *spf, *usingScreen, *isMedia,
	     static_cast<unsigned long>(*audioFormat));
	switch (*ct)
	{
	case 2:
		type = 0x20;
	...
	{
		write_coverart(coverart_filename.c_str(), empty_image, sizeof(empty_image));
	}
}
...
extern "C" void audio_set_coverart(void* cls, const void* buffer, const int buflen)
{
...
extern "C" void audio_set_progress(void* cls, const unsigned int start, const unsigned int curr, const unsigned int end)
{
	const int duration = static_cast<int>(end - start) / 44100;
	const int position = static_cast<int>(curr - start) / 44100;
	const int remain = duration - position;
...
{
	char dmap_tag[5] = {};
	auto metadata = static_cast<const unsigned char*>(buffer);
	int datalen;
	int count = 0;
	...
	while (buflen >= 8)
	{
		...
		metadata += 8;
		buflen -= 8;
		process_metadata(count, dmap_tag, metadata, datalen);
		metadata += datalen;
		buflen -= datalen;
	}
	if (buflen != 0)
...
extern "C" void register_client(void* cls, const char* device_id, const char* client_pk, const char* client_name)
{
	...
	registered_keys.emplace_back(client_pk);
	if (strlen(pairing_register.c_str()))
	{
		FILE* fp;
		if (fopen_s(&fp, pairing_register.c_str(), "a") == 0)
		{
...
extern "C" void log_callback(void* cls, const int level, const char* msg)
{
	switch (level)
	{
	case LOGGER_DEBUG:
...
static int start_raop_server(unsigned short display[5], unsigned short tcp[3], unsigned short udp[3], bool debug_log)
{
	...
	if (display[0]) raop_set_plist(raop, "width", display[0]);
	if (display[1]) raop_set_plist(raop, "height", display[1]);
	if (display[2]) raop_set_plist(raop, "refreshRate", display[2]);
	if (display[3]) raop_set_plist(raop, "maxFPS", display[3]);
	if (display[4]) raop_set_plist(raop, "overscanned", display[4]);
	...
	if (require_password) raop_set_plist(raop, "pin", pin);
...
static void read_config_file(const char* filename, const char* uxplay_name)
{
	...
	options.emplace_back(uxplay_name);
		...
		while (std::getline(file, line))
		{
				...
				else
				{
					...
					if (line[i] == endchar)
					{
						if (in_quotes)
						{
							...
							if (i + 1 < static_cast<int>(line.size()) && line[i + 1] != ' ') continue;
						}
						line[i] = '\0';
						is_part_of_item = false;
					}
			...
			while (std::getline(iss, token, '\0'))
			{
				if (!token.empty())
				{
					if (first)
					{
						options.push_back(option_char + token);
						first = false;
					}
					else
					{
						options.emplace_back(token.c_str());
					}
				}
			}
		}
	...
	if (options.size() > 1)
	{
		...
		auto argv = static_cast<char**>(malloc(sizeof(char*) * argc));
		for (int i = 0; i < argc; i++)
		{
			argv[i] = const_cast<char*>(options[i].c_str());
		}
...
int main(const int argc, char* argv[])
{
	...
	{
		if (std::ifstream file(pairing_register); file.is_open())
		{
			int clients = 0;
			std::string key;
			std::string line;
